...
<body>
    ...
    $result = mysqli_query($conn, "
        SELECT m.*, t.type_name, 
               (SELECT COUNT(*) FROM rent WHERE movie_ID = m.movie_ID AND rent_status = 'ยังไม่คืน') AS pending_rentals
        FROM movie m
    ...
    <table>
        ...
        <tr>
            ...
            <td><?= htmlspecialchars($row['movie_stock']); ?></td>
            <td><?= $row['movie_Status'] ? '🟢 ขายอยู่' : '🔴 หยุดขาย'; ?></td>
            <td>
                <a href="edit_movie.php?id=<?= $row['movie_ID']; ?>" class="btn btn-success">✏️ แก้ไข</a>
